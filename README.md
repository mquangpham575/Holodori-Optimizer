...
| Deploy    | Vercel (frontend) + Northflank (Node backend + Postgres)           |
...
│   │   └── etl/           # HolodoriDB master-table sync, card-art (card-art-cdn.ts) and video (card-media.ts) mirrors + manual scripts
│   ├── tests/             # integration tests (node:test + tsx) + fixtures
│   ├── database.json      # Dev-mode JSON data store (PostgreSQL in prod)
│   └── dist/              # Compiled output (gitignored)
├── frontend/              # Vite + React 19 app
│   ├── src/
│   │   ├── components/    # TeamBuilder, CharacterDB, Guides, AdminDashboard, Home, Navbar, CardArt, CardStage
│   │   ├── search_worker.js   # Ported Holodori Optimizer engine (Web Worker)
│   │   ├── allCards.js        # Builds the card pool + art/media URLs from raw character data
│   │   ├── cardMedia.js       # Animation/signature preferences and voice preloading
│   │   ├── charUtils.js       # Type icons and card helpers
│   │   ├── data.js            # Static catalog snapshot (fallback seed; regenerated by the sync)
│   │   └── context/           # i18n — 11 languages, URL-prefixed routes
│   ├── public/            # Static assets: portraits, type icons, blog articles
│   └── dist/              # Production build output
├── docs/                  # Permission-request drafts (optimizer, HolodoriDB)
├── tests/e2e/             # Playwright smoke tests (hermetic, fixtures included)
...
| `npm run server` / `npm run client` | Aliases: compiled backend / Vite dev server alone |

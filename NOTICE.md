...
| Card artwork | `card_art` / `card_art_full` tables, `frontend/public/images/cards*` | Third-party game artwork owned by COVER Corp. / QualiArts and the artists. The framed card art comes from the optimizer bundle; the full illustrations are read from `cdn.holodori.dev`, the CDN of the community site holodori.best, which serves them without authentication. The 5-star animation and signature overlay are copied **on demand**: the first time a visitor opens a card the backend redirects them to the CDN and downloads the two videos in the background (the animation re-encoded to about a third of its size), and later visits are served from our own store; only cards somebody opened are ever stored, up to `CARD_MEDIA_MAX_MB`. This exists to make the first view fast, and `CARD_MEDIA_MIRROR=false` switches it off (back to pure CDN redirects). The voice line is not copied: visitors' browsers stream it from the CDN when a card is maximised. The signature is composited with plain canvas drawing (the CDN only lets holodori.best read its files from script). We download each image once and re-check weekly, but the CDN is someone else's infrastructure: consider asking its operator for permission and credit them. A draft request is in `docs/permission-request-holodoridb.md`. Set `CARD_ART_CDN_BASE=""` (backend) and `VITE_CARD_VIDEO_BASE=""` (frontend build) to switch this off, and `CARD_ART_BUNDLE=false` to also drop the optimizer's bundled art. Rights remain with the rights holders; remove on request. |

...
| Card artwork | `card_art` / `card_art_full` tables, `frontend/public/images/cards*` | Third-party game artwork owned by COVER Corp. / QualiArts and the artists. The framed card art comes from the optimizer bundle; the full illustrations are read from `cdn.holodori.dev`, the CDN of the community site holodori.best, which serves them without authentication. The 5-star animation, signature overlay and voice line are not copied: visitors' browsers stream them from the same CDN when a card is opened (the CDN only lets holodori.best read them from script, so the signature is composited with plain canvas drawing rather than by reading pixels). We download each image once and re-check weekly, but the CDN is someone else's infrastructure: consider asking its operator for permission and credit them. A draft request is in `docs/permission-request-holodoridb.md`. Set `CARD_ART_CDN_BASE=""` (backend) and `VITE_CARD_VIDEO_BASE=""` (frontend build) to switch this off, and `CARD_ART_BUNDLE=false` to also drop the optimizer's bundled art. Rights remain with the rights holders; remove on request. |

// VENDORED: this file is a port of the holodori-optimizer search engine
// (github.com/ace-ks-dev/holodori-optimizer). It runs inside a Web Worker
// (CARDS is injected at build time) to power team recommendation.
// Minimal adaptations only — the scoring math is upstream reference code.
// NOTE: upstream is "all rights reserved" (no open-source licence); obtain the
// author's permission or replace this file before public distribution.
"use strict";
...
function representativePerms(canon){
...
function optimize(params){
  ...
  for(let q=0;q<beam.length;q++){const canon=beam[q].ids;calcCounts(canon,COUNTS);const baseCounts=new Int32Array(COUNTS);let best=-Infinity;for(const perm of representativePerms(canon)){const ids=perm.map(i=>canon[i]);mapCounts(baseCounts,perm,MAPPED_COUNTS);const ev=evaluateOrder(ids,MAPPED_COUNTS,params,false);if(ev&&ev.score>best)best=ev.score;}if(best>-Infinity)refinedHeap.push({score:best,ids:canon.slice()});if(!params.suppressProgress&&(q%100===0||q===beam.length-1))self.postMessage({type:'progress',phase:'Refining scalable-search finalists',done:q+1,total:beam.length,valid:counters.completeScreened,best:refinedHeap.a.length?Math.max(...refinedHeap.a.map(x=>x.score)):0});}
...
function bestNextCard(params){
  ...
  for(let i=0;i<refineCount;i++){const x=coarse[i],c=CARDS[CARD_BY_ID.get(x.cardId)],p=structuredClone(raw);p.ownedKeys=[...owned,c.key];p.requiredCards=[...(raw.requiredCards||[]),c.id];p.searchQuality='upgrade';p.topN=1;p.suppressProgress=true;try{const out=optimize(p),r=out.results[0];if(r)refined.push({cardId:c.id,score:r.score,result:r});}catch{}self.postMessage({type:'upgradeProgress',phase:'Testing the strongest upgrade candidates',done:i+1,total:refineCount});}
